...
public:
    ...
    reference operator()(size_type const i, size_type const j) noexcept { return data[i * cols + j]; }

    T operator()(size_type const i, size_type const j) const noexcept { return data[i * cols + j]; }

    reference operator()(Point const& p) noexcept { return this->operator()(p.x, p.y); }

    T operator()(Point const& p) const noexcept { return this->operator()(p.x, p.y); }
...
template <typename T> std::ostream& operator<<(std::ostream& os, Mat<T> const& mat)
{
